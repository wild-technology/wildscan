...
The registry DATA now lives in modules/cameras.json (per-camera /
...
def _assert_parity() -> None:
    ...
    though the module's own docstring calls that file the place
...
def ensure_calibration_sidecars(image_root: str) -> tuple[int, int]:
    ...
    COLMAP-bridge stereo path (VOYIS) the operator found sidecar
