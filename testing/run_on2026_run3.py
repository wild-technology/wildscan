r"""ON2026 run3: planned campaign from M:\ON2026_run3 (2026-08-28).

Campaign requirements (all mandatory):
  ...
  - align: Brown3 (specified; C-20260730-09 context recorded), High
  ...
  - per-zone aligns -> grow -> merges, HONORING the 34,000-camera merge
    ...
    merge scene.
  - NO model stage in run3 (dense/meshing runs separately).
...
WORKDIR = os.path.join(RUN3, "_work")
...
ALIGN_BAT = os.path.join(WORKDIR, "tools", "Run3PoolCalibAlign.bat")
...
ZONE_CAP = 10_000               # project decision: zones <= 10,000 images
...
#            - metric gate FAILED, campaign aborted per the campaign rule.
...
def guard_campaign_arm(arm):
    ...
    rec_path = os.path.join(WORKDIR, "campaign_arm.json")
...
def stage_fixture(arm):
    verdict_path = os.path.join(WORKDIR, "fixture_verdict.json")
    ...
    for s in stems:
        for eye in ("L_", "R_"):
            ...
            members.append(p)
    fzone = os.path.join(WORKDIR, "fixture", "zones", "zone_fx")
    ...
    clear_markers()
    out_root = os.path.join(WORKDIR, "fixture")
    ...
    if verdict["verdict"] != "PASS":
        abort("FIXTURE GATES FAILED - new-settings import chain does not "
              ...
              "(campaign rule). Evidence: fixture_verdict.json + "
...
def stage_pair_repair():
    ...
    Then enforce the <= ZONE_CAP invariant mechanically."""
    master = {}
    ...
    for zd in zone_dirs():
        ...
        if len(paths) > ZONE_CAP:
            abort(f"{zname}: {len(paths)} images exceeds the zone cap "
...
def stage_merges():
    ...
    plan = {"total_unique_cameras": total_unique, "ceiling": CEILING,
            "groups": groups}
    json.dump(plan, open(os.path.join(WORKDIR, "merge_plan.json"), "w",
...
def stage_manifest(plan, merge_results):
    manifest = {
        ...
        "settings": {
            ...
            "distortion": "Brown3 (specified; C-20260730-09 measured "
        ...
        "fixture_verdict": os.path.join(WORKDIR, "fixture_verdict.json"),
        "merge_plan": plan,
        "groups": {},
    }
...
def main():
    # schtasks-launched trees default to BELOW-NORMAL priority; the operator
    ...
    for d in (LOGS, WORKDIR, COMPONENTS, GROWN, MERGED, CONFIG, RSCMD_DIR):
        os.makedirs(d, exist_ok=True)
    arm = calib_arm()
    log(f"driver start: {CODE} run3 from {RUN3} (planned, "
