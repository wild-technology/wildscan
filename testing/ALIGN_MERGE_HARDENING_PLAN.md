...
U7 remains OPEN — tonight's proxy is GUI screenshot verification
of the assembly project.

STATUS UPDATE (2026-07-23, 9-boot probe session; details in FINDINGS):
...
graduates into FINDINGS (with its result) once run. Cells are
...
- **U20 identity-capture loop, live end-to-end.** Covers the manifest
  open risks from the smoke run: (a) stem-named (non-ordinal) sidecars
...
- **U7 georeferenced-merged verification, CLI-observable.** Operator GUI
  ...
  status/residuals headless? status log warns exportRegistration without
...
## Forum mining track (standing, project decision 2026-07-23)
...
- Every adopted gem goes to FINDINGS with URL, author + staff/user
...
5. The simplify "noise"/"smooth" params are placeholder percentages
   (70/80 rel) pending custom GUI presets.
