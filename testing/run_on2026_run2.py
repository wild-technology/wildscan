...
approved fully unattended 2026-08-08 (goal session: features.json
...
Work directory: M:\ON2026_run2\_work.
"""
...
WORKDIR = os.path.join(RUN2, "_work")
...
MIN_COMPONENT = 10          # flag-pole pockets (guidance: pair 10-15; low end)
MERGE_MIN_SIZE = 10
# PROJECT DECISION 2026-08-09 ~08:00: raised from 34,000 (C-20260802-01)
...
def regen_master(ori_acc, out_path):
    ...
    tmp = os.path.join(WORKDIR, f"navgen_{ori_acc}")
...
def stage_ab():
    verdict_path = os.path.join(WORKDIR, "ab_verdict.json")
    ...
    tight_master = os.path.join(WORKDIR, "flight_log_ori10_master.txt")
    if not os.path.isfile(tight_master):
        regen_master(10, tight_master)
    tight_zone_log = os.path.join(WORKDIR, f"{zone_name}_ori10_log.txt")
    filter_log(tight_master, names, tight_zone_log)

    cells = {}
    for tag, zlog in (("ori90", zone_log), ("ori10", tight_zone_log)):
        root = os.path.join(WORKDIR, f"ab_{tag}")
    ...
    verdict = {"adopted": "ori10" if adopt_tight else "ori90",
               "rule": "pre-approved 2026-08-08 (ties -> 90)",
               "cells": cells,
...
def stage_features():
    plan_path = os.path.join(WORKDIR, "features_plan.json")
    boxes, default, confirmed = feature_merge.load_feature_boxes(FEATURES_JSON)
    if not confirmed:
        abort("features.json is not confirmed - review gate")
    ...
    manifest = {
        ...
        "coordinate_frame": {
            ...
            "absolute_georeference": "none - local frame; UTM via USBL "
                                     "fit available later (open question Q3b)",
...
def main():
    for d in (LOGS, WORKDIR, MERGED, MODELS):
        os.makedirs(d, exist_ok=True)
    log(f"driver start: {CODE} from {RUN2} (unattended, approved "
        "2026-08-08)")
    guard_disk()
    stage_zoning()
    ab = stage_ab()
