r"""ON2026 calibration ladder (project decision 2026-08-08).
...
M:\rs_cache), and after the field finding that same-name XMP
sidecar AUTO-IMPORT is unreliable: every cell here delivers calibration
through EXPLICIT commands (FINDINGS 2026-08-08), on the quiet RS2
...
Verdict: M:\ON2026_run2\_work\calib_ladder\ladder_verdict.json.
"""
...
LADDER = os.path.join(WORKDIR, "calib_ladder")
TREE = os.path.join(WORKDIR, "calib_zone1")          # rs_images + zone log
...
def main():
    ...
    old = os.path.join(WORKDIR, "calib_test")
