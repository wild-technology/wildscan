r"""Overnight workbench seed-growth campaign (project decision 2026-08-11).

Drives the LIVE GUI instance RSGUI (operator's workbench scene) via the
attach-only NightGrow.bat primitives - never boots, never -newScene,
never -quit; the GUI stays active all night. Log:
M:\ON2026_run2\logs\night.log; workdir M:\ON2026_run2\_work\night.

Plan, implemented stage by stage:
  W  wait for the operator's in-flight alignment to finish (progress file
  ...
  A  add accepted orphans + flight-log priors; added images set to
     ALL FEATURES (aligFeaturesMode=2) as specified - registered images
     keep their existing feature source
  S  seed-growth passes: disable ALL, enable ONLY small components +
     orphans (largest component stays disabled, as specified), align,
...
WORK = os.path.join(RUN2, "_work", "night")
...
def _mem_gb():
    ...
    and a 30 s peak sampler (decision 2026-08-11: track time AND memory
    per step)."""
    try:
        out = subprocess.run(
...
def night(mode, *args):
    ...
    component count - note 2026-08-11; every mutating mode already
...
def operator_align_idle(stale_minutes=12):
    """True when the operator's alignment is done: progress file stale or
...
def stage_wait():
    log("W: waiting for the operator's alignment to go idle "
        "(progress stale >12 min)")
    while not operator_align_idle():
        time.sleep(120)
...
def main():
    ...
    excluded_stems = set()
    # D: delete the second-largest component (project decision).
    ...
    if len(comps) >= 2:
        ...
        if victim_gone and largest_ok:
            ...
            # FALLBACK (goal-preserving): stop theorizing about
            ...
            # solve-level equivalent of deletion; the operator can delete
