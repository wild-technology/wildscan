...
requested 2026-07-25 ("have you tried running everything with
...
**STATUS ELEVATION (reframe, 2026-07-25): this plan is the
...
1. **The custom 13-column flight-log format was NEVER INSTALLED** —
   ...
   date ("Global camera prior settings" in the GUI — confirmed
   by observation). FIXED: format merged into
...
4. **Empirical calibrations from the fresh run's 4,405 solved
   ...
   - Cinema: 16.37 mm 35-eq (p10–p90: 16.24–16.53), k1 −0.053
     (prior said 17.0 — close; the stated "23 mm" is the physical lens)
   - Port: 15.37 mm 35-eq (15.23–15.52), k1 −0.324 (strong barrel —
     fisheye through a brown3 model; prior said 14.0, the stated 16 mm
     physical ≈ right)
...
RealityScan.log's per-align RMS lines; if absent, operator reads the GUI
...
| PD-4 | Z1 | Division + orientation@15° + 1/1/0.1 | ≥97.0%, fewer comps | DONE — **COLLAPSED: 669/4540 (14.7%), ONE comp = the BOW box.** The hull band did not solve at all. Ran under heavy contention (concurrent tests, ~4 GB free) — partially confounded **[CONTAMINATED 2026-07-26: ran with Euler order and Camera mount UNPINNED - see FINDINGS contamination flag. Count stands; attribution to orientation priors does not.]** |
...
rig ground truth from the operator (or a solve WITH orientation priors
...
delivered assembly stays as-is (evaluation gate).
...
- Euler order / camera-mount convention for the imported YPR is
  ...
  image's orientation prior vs its flight-log row (operator, 1 minute)
  before any Z1 spend.
