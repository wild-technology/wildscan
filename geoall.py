...
# Path prompt defaults. No machine-specific paths: the first run asks, and
# SettingsStore offers the last-entered values from rs_settings.json after that.
...
def get_camera_pitch_accuracy(filename: str, assume_pitch: bool = True,
    ...
    convention (ASSUMED_MOUNT_DEFAULTS, adopted 2026-08-31): 10 deg down
...
def get_camera_pitch_offset(filename: str, assume_pitch: bool = True,
    ...
    A MEASURED mount always wins. With none, this falls back to the house
    convention (ASSUMED_MOUNT_DEFAULTS: 10 deg down, adopted 2026-08-31),
...
def main(argv: list[str] | None = None):
    ...
    for label, path in (("image base directory", image_base_dir),
                        ("ROV data directory", rov_data_dir)):
        if not path or not os.path.isdir(path):
            raise SystemExit(f"{label} not found: {path!r} - pass it with "
                             "--image-base-dir / --rov-data-dir or enter it "
                             "at the prompt")
    if not output_dir:
        raise SystemExit("output directory not set - pass --output-dir or "
