...
# FINDINGS — unified COLMAP fact base (colmap_studio ⊕ itsmagicIswear)
...
- **C-20260721-01** — Dense reconstruction MUST run in LOCAL coordinates.
  ...
  magnitude analysis. Source: status log §1 bug 1.
- **C-20260721-02** — COLMAP 4.x keeps AUTHORITATIVE poses in `frames.txt`
  ...
  cameras). Source: status log §1 bug 2.
...
- **C-20260721-09** — colmap_studio's own recipe
  ...
  burden of proof has flipped against preprocessing. Source: status log §4,
...
- **F-20260723-34** — Priors can hurt: RS_main's NA167 zone_13 A/B measured
  ...
  per dataset, never assumed beneficial. Source: `RS_main/status log`.
...
- **F-20260722-02** — SILENT FAILURE: multi-GPU `feature_extractor`
  ...
  Source: variant_a DB forensics; also status log §8.
- **F-20260722-03** — SILENT FAILURE: `rig_config.json` `camera_params` MUST
  ...
  (RH0041 pilot + LilyJean variant_a). Source: status log §6; variant_a
...
- **C-20260723-01** — Second head-to-head, monolith validation subset
  ...
  rig/high-overlap data. Source: status log §7.
...
- **C-20260723-02** — `global_mapper`'s "Decomposing relative poses" is a
  ...
  parallel stage. Upstream filing candidate. Source: status log §7 run notes.
- **C-20260724-01** — Ceres 32-bit nnz overflow: monolith global positioning
  ...
  rerun ≈ 3–5 h. Source: status log §7, global.log 2026-07-24 04:26.
...
- **C-20260724-02** — Local binary verification (2026-07-24): colmap-caspar
...
- **C-20260723-03** — OpenBLAS ROOT-CAUSE FIX STAGED, NOT APPLIED (verified
  ...
  match numpy exactly. Swap procedure + rollback in status log §6. Caveat:
...
- **C-20260721-05** — zone_9 residual forensics
  ...
  points were never refined. Source: status log §3,
...
- **C-20260723-04** — Sampling-rate analysis (measured on the monolith
  ...
  added later. Source: status log §7.
...
- **C-20260724-03** — E: NVMe ("4TB-RUMI-F") FAILING: stornvme controller
  ...
  OperationalStatus="Full Repair Needed" (checked 2026-07-24).
  MITIGATION (2026-07-24): rescue copy of database.db +
  ...
  (raw row count; status log's "1,704,507 verified pairs" is the
  ...
  Source: status log §7; Get-Volume + Get-WinEvent + robocopy log + sqlite
