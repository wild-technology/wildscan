...
Written 2026-07-24 in response to the status log task "onboard, then produce
...
manifests — pure analysis, no RealityScan — resolves the operator's
...
it refutes, the Q5-batcher common-image-pool change (status log P1 #5)
...
- **Q8 (per-component models)** — drive `GenerateModel` from the
  evaluation gate: operator selects surviving components, or all above a
