...
era-specific filename families (confirmed 2026-07-23):
...
Specified default recipe (GenerateModel.bat): Generate High ->
...
- `-simplify` takes params.xml exported from the GUI Simplify tool.
  ...
  (80% rel) are placeholders derived from the 50% template - if custom
  GUI presets exist for "noise" and "smooth"
