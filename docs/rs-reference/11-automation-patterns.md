...
**Citation keys used here.** `FINDINGS <date>` = FINDINGS entry with
that date. `NA167 #n` = the frozen numbered log the NA167 log (#1–31).
`NA167 Bn` = the bug list in the NA167 notes §2 — **note that source
...
`status log <item>` = the status log. Help topics are cited by their converted path, e.g.
...
hand-over, not at completion [VERIFIED: NA167 notes §1; status log 2026-07-21].
...
| `-set "appProcessExecCmd=…"` with `\"` around the VBS path | Unquoted paths silently disabled **all** error detection when the checkout path contained spaces [VERIFIED: status log overhaul item 4, 2026-07-21] |
...
| `appAutoClearCache` **untouched** | Retention is operator policy, not a per-run decision [VERIFIED-as-decision: startRealityScan.bat]. Values are a retention age in days; `999999` = never clear, `0` = clear all, default **`7` (items older than one week)** [OFFICIAL: tutorials/setkeyvaluetable] |
...
- **[CONTRADICTED — internal]** `ARCHITECTURE.md` hard rule 3 documents the shutdown bound as
  300 s; the code uses `SHUTDOWN_VERIFY_TIMEOUT_SECONDS = 900` and the status log
  ...
  [VERIFIED: realityscan_cli.py lines 190–191; status log verification item 5].
...
- Shutdown timing has only been verified on **small** scenes
  [VERIFIED: status log verification item 5, 2026-07-21].
...
[VERIFIED: FINDINGS 2026-07-21; status log verification item 2, 2026-07-21].
...
- **Fully qualify Windows utilities.** Every workflow writes
  ...
  `%SystemRoot%\System32\find.exe` [VERIFIED: status log verification item 2, 2026-07-21].
...
- **Invoke the `.bat` by absolute path with no `cmd /c` prefix** from Python. A bare script
  ...
  `.bat` quoting correctly on its own [VERIFIED-as-fix: realityscan_cli.py, status log
  2026-07-21]:
...
- **`CREATE_NO_WINDOW` on every helper subprocess.** Console-subsystem children
  ...
  [VERIFIED: `_NO_WINDOW` in realityscan_cli.py, operator report 2026-07-23].
...
each time — hundreds of flashing terminals over a long run (operator report, 2026-07-23).
...
FIFO ordering VERIFIED: NA167 notes §1].
...
| A GUI session holding the deliverable project open while a workflow `-load`s it | **Blocks in practice.** An interactive GUI session holding `H2024_Final_Assembly` open blocked the export probe [VERIFIED-as-incident: status log 2026-07-29] |
...
[VERIFIED: NA167 #12 / B4 and NA167 notes §1, 2026-07-23].
...
- **Two disk columns, deliberately.** `disk_free_gb` watches the drive holding the trace
  ...
  and the right *instance* of it [VERIFIED-as-fix + lesson: status log 2026-07-27].
...
| `results_<inst>.log` growth | Internal heartbeat processes write it too [VERIFIED: status log 2026-07-21] |
...
- **Component reimport does not carry non-member images.** Orphans (never-registered
  ...
  flight log is imported [VERIFIED: FINDINGS 2026-07-23, confirmed]. Those orphans
...
> [VERIFIED: NA167 #23; restated in ARCHITECTURE.md, status log, MERGE_STRATEGY_REPORT]

Basis: **only registered cameras get pose entries**, so counting pose-bearing `.xmp`
sidecars is a reliable registration census [VERIFIED: NA167 notes §1]. A
...
[VERIFIED: NA167 notes §1]. Membership must be *derived*, and the derivation rests
on one naming fact:
...
- **`-exportXMP` covers "components created in the last alignment"** and the components
  ...
  scene built by `-importComponent` [VERIFIED: status log verification item 2, 2026-07-21].
...
[VERIFIED-as-decision: status log 2026-07-29].

**Make the decision a pure function and have the tests drive it.** Two of the
merge driver's own regression tests re-implemented the accept arithmetic and would have kept passing had the
...
2. **Flight-log arithmetic goes wrong.** A copied image is two physical files with one
   ...
   rows — a 638-row gap that looks like missing nav [VERIFIED-as-symptom: status log loose
   end #6].
...
- **Export names must be unique per attempt.** `peel_index` restarts at 0 every attempt, so
  ...
  [VERIFIED-and-fixed: FINDINGS 2026-07-28; status log 2026-07-29].
...
- **`-mergeComponents` is a no-op with a single component, and its asynchronous
  ...
  that predates it [VERIFIED: status log verification item 2, 2026-07-21].
...
- **A multi-component terminal state is a correct outcome.** Zones are cut on image
  **density** and are blind to feature boundaries; H2023 contains two discrete physical
  ...
  disjoint features as merge failure [VERIFIED-as-decision: FINDINGS 2026-07-24].
...
specified eight-step recipe, with the literal model names:
...
|---|---|---|
| 1 | Gate completion on `results_<inst>.log` growing | Internal heartbeat processes write it; a check built on it raced ahead of a running `-align` [VERIFIED: status log 2026-07-21] |
...
| 32 | Leave `appProcessExecCmd` paths unquoted | Silently disabled all error detection when the checkout path contained spaces [VERIFIED: status log 2026-07-21] |
...
| 44 | Edit a `.bat` while a workflow is running | cmd reads the file by byte offset; a mid-run edit corrupts execution. Review-only during live runs [VERIFIED-as-policy: status log 2026-07-27] |
