...
## Arrived 2026-08-07 (approved archive quarantine)
...
| `AlignImageList.bat` | Retired by project decision 2026-08-07 with zero callers — superseded by `AlignZone.bat` + `GrowZone.bat`. Carries an unfixed status log SHOULD-FIX by design (no AlignmentParams application, no deselect before exports; status log 2026-07-24 clean-sweep review backlog). |
| `SequentialAlignGrow.bat` | Retired by project decision 2026-08-07 with zero callers — superseded by `AlignZone.bat` + `GrowZone.bat`. Same unfixed-by-design status log SHOULD-FIX as `AlignImageList.bat`. |
